          ...
          <div key={question.id} className={`col-md-12 wow animate__animated animate__fadeIn animate__delay-0.5s ${dropStyles.sana_drop_question}`} >
            <div className={dropStyles.sana_DropQues_container}>
              <p onClick={() => handleOpen(question.id)}>
                <span className={`${dropStyles.question_number} ${openQuestionIds.includes(question.id) ? dropStyles.opened : "" }`}>
                  {index + 1}.
                </span>{" "}
                {question.title}
                <span className={` ${dropStyles.sana_chevron_icon} ${openQuestionIds.includes(question.id) ? dropStyles.rotate : "" }`}>
                  <FaPlus />
                </span>
              </p>
            </div>
            <div className={` ${dropStyles.sana_dropdown_content} ${openQuestionIds.includes(question.id) ? dropStyles.open : "" }`}>
